...
public class QuestController : Controller
{
    ...
    public QuestController(AppDataService dataService, GameService gameService, SocialService socialService)
    {
    ...
    public IActionResult Details(int id)
    {
        ...
        ViewBag.Reviews = _socialService.GetQuestReviews(id);
        ViewBag.AverageRating = _socialService.GetAverageRating(id);
        ViewBag.CanReview = user != null && user.QuestProgresses.Any(x => x.QuestId == id && x.IsCompleted);
        return View(quest);
    }
    ...
    public IActionResult CheckAnswer(int id, int pointId, string answer)
    {
        ...
        var currentAttempts = HttpContext.Session.GetInt32(GetAnswerAttemptsSessionKey(id, pointId)) ?? 0;
        ...
        if (Normalize(currentPoint.Answer) == Normalize(answer))
        {
            ...
            {
                _gameService.RegisterPreciseAnswer(userId.Value);
            }
    ...
    public IActionResult Review(int id, int rating, string comment)
    {
        if (!IsRegisteredUser())
        {
            TempData["Error"] = "Сначала войди в аккаунт, чтобы оставить отзыв.";
            return RedirectToAction(nameof(Details), new { id });
        }
        ...
        if (_socialService.UpsertQuestReview(userId.Value, id, rating, comment))
            TempData["Success"] = "Отзыв и оценка сохранены.";
        else
            TempData["Error"] = "Отзыв можно оставить только после завершения квеста. Оценка должна быть от 1 до 5.";
