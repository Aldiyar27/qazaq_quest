...
public class GameService
{
    ...
    public AppUser? GetCurrentUser(HttpContext httpContext)
    {
        ...
        return _dbContext.Users
            ...
            .Include(x => x.QuestReviews)
            .Include(x => x.DailyChallengeProgresses)
    ...
    public void AdvanceStep(Guid userId, Quest quest, bool completed)
    {
        ...
        else
        {
            ...
            if (!progress.IsRewardClaimed)
            {
                ...
                UpdateDailyChallengeProgress(userId, "CompleteQuests", 1);
            }
        }
    ...
    public void RegisterPreciseAnswer(Guid userId)
    {
        UpdateDailyChallengeProgress(userId, "PreciseAnswers", 1);
        _dbContext.SaveChanges();
    }
    ...
    private void UpdateDailyChallengeProgress(Guid userId, string challengeType, int increment)
    {
        var challenges = _dbContext.DailyChallenges
            .Where(x => x.IsActive && x.ActiveDateUtc.Date == DateTime.UtcNow.Date && x.ChallengeType == challengeType)
            .ToList();

        foreach (var challenge in challenges)
        {
            var progress = _dbContext.DailyChallengeProgresses.FirstOrDefault(x => x.UserId == userId && x.DailyChallengeId == challenge.Id);
            if (progress == null)
            {
